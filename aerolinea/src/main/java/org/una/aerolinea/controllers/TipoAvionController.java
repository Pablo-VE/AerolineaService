...
public class TipoAvionController {
    ...
    public ResponseEntity<?> create(@RequestBody TipoAvionDTO avion,  BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                return new ResponseEntity(tipoService.create(avion), HttpStatus.CREATED);
            } catch (Exception e) {
    ...
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @RequestBody TipoAvionDTO avionModified, BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                Optional<TipoAvionDTO> tAvionUpdated = tipoService.update(avionModified, id);
                if (tAvionUpdated.isPresent()) {
                    return new ResponseEntity(tAvionUpdated, HttpStatus.OK);
                } else {
